...
public class CookingSubmissionsFragment extends Fragment {

    private CommunicationInterface3 callback;

    private List<submission> Submissions;
    private RecyclerView lister;
    private CookingSubmissionsAdapter submissionsAdapter;
    private String uid;
    ...
    private void load_submissions_from_firebase(@NonNull DataSnapshot dataSnapshot) {
        for(DataSnapshot CHILDER: dataSnapshot.getChildren()){
            final Object rating = CHILDER.child("mysubmissions").child("cookingcomp").child("rating").getValue();
            if(CHILDER.child("mysubmissions").child("cookingcomp").child("rating").exists()){
                final Object submittersid = CHILDER.getKey();
                final Object ratings = CHILDER.child("mysubmissions").child("cookingcomp").child("ratings").getValue();
                final Object name = CHILDER.child("name").getValue();
                final Object imageoncloud = CHILDER.child("mysubmissions").child("cookingcomp").child("imageoncloud").getValue();
                //Object description = achild.child("description").getValue();
                if(rating!=null && imageoncloud!=null && name!=null && submittersid!=null && ratings!=null) {
                    ...
                    Submissions.add(new submission() {{
                        ...
                        IMAGEONCLOUD = imageoncloud.toString();
    ...
    private void fill_recyclerview() {
        if(getView()!=null) {
            ...
            submissionsAdapter = new CookingSubmissionsAdapter(getContext(), Submissions);
